...
{
    [SerializeField] private Transform axis; // ось
    [SerializeField] private Floor floorPrefab; // прифаб


    [Header("Settings")]
    [SerializeField] private int defaultFloorAmount; // изначальные значения 
    [SerializeField] private float floorHeight;
    [SerializeField] private int ammountEmptySegment;
    ...
    public void Generate(int level)
    {
        ...
        for (int i = 0; i < floorAmount; i++)
        {
            ...
            if (i == 0)
            {
                floor.SetFinmishSegment();
            }

            if (i > 0 && i < floorAmount - 1)
            {
                floor.SetRandomRotation();
                floor.AddEmptySegment(ammountEmptySegment);
